...
def format_title_for_platform(
    ...
    if platform == "wework":
        ...
        title_prefix = "[新] " if title_data.get("is_new") else ""

        if show_source:
            result = f"{title_prefix}**{title_data['source_name']}** - {formatted_title}"
        else:
        ...
        meta_info = []
        if rank_display:
            meta_info.append(f"`{rank_display}`")
        if title_data["time_display"]:
            meta_info.append(f"<font color=\"comment\">{title_data['time_display']}</font>")
        if title_data["count"] > 1:
            meta_info.append(f"<font color=\"info\">×{title_data['count']}</font>")
...
def render_html_header(
    is_daily_summary: bool, mode: str, total_titles: int, hot_news_count: int
) -> str:
    """渲染HTML头部、样式和报告信息"""
    now = get_beijing_time()
...
def render_html_stats(report_data: Dict) -> str:
    """渲染统计数据区域"""
    html = ""
...
def render_html_new_section(report_data: Dict) -> str:
    """渲染新增新闻区域"""
    html = ""
...
def render_html_footer(update_info: Optional[Dict] = None) -> str:
    """渲染页脚"""
    html = """
            </div>
...
def split_content_into_batches(
    ...
    base_header = ""
    # 获取词汇数量
    keyword_count = len([stat for stat in report_data["stats"] if stat["count"] > 0])
    base_header += f"{total_titles} 条新闻 | {keyword_count} 个热点 | {now.strftime('%m-%d %H:%M')}\n\n"

    base_footer = ""
    if update_info:
        base_footer = f"\n\n---\n\n> 新版本 {update_info['remote_version']}"
    ...
    if report_data["stats"]:
        ...
        # 逐个处理词组（每个关键词独立成为一个批次）
        for i, stat in enumerate(report_data["stats"]):
            # 如果不是第一个关键词，先完成当前批次，然后开启新批次（不带 base_header）
            if i > 0 and current_batch_has_content:
            ...
            if i == 0:
                ...
                # 检查第一个关键词是否超过限制
                if (
                    len(test_batch.encode("utf-8")) + len(base_footer.encode("utf-8"))
                    >= max_bytes
                ):
                    # 超过限制，base_header 单独成批次
                    if base_header:
                        batches.append(base_header.rstrip() + base_footer)
                    current_batch = word_with_first_news
                else:
            ...
            for j in range(start_index, len(stat["titles"])):
                ...
                if (
                    ...
                    if current_batch_has_content:
                        batches.append(current_batch + base_footer)
                    # 新批次继续这个关键词的内容（不显示关键词标题）
                    current_batch = news_line
                    current_batch_has_content = True
                else:
    ...
    if report_data["new_titles"]:
        new_header = f"\n\n## 本次新增\n\n"
        new_header += f"> 共 <font color=\"info\">{report_data['total_new_count']}</font> 条新增热点\n\n"
        ...
        for source_data in report_data["new_titles"]:
            ...
            source_header += f"> 新增 {len(source_data['titles'])} 条\n\n"
...
def send_to_wework(
    ...
    try:
        preview_response = requests.post(
            webhook_url, headers=headers, json=preview_payload, proxies=proxies, timeout=30
        )
        if preview_response.status_code == 200:
            result = preview_response.json()
            if result.get("errcode") == 0:
                print(f"企业微信预告消息发送成功 [{report_type}]")
                time.sleep(1)  # 预告消息后等待1秒
            else:
                print(f"企业微信预告消息发送失败 [{report_type}]，错误：{result.get('errmsg')}")
        else:
            print(f"企业微信预告消息发送失败 [{report_type}]，状态码：{preview_response.status_code}")
    except Exception as e:
        print(f"企业微信预告消息发送出错 [{report_type}]：{e}")
    ...
    for i, batch_content in enumerate(batches, 1):
        ...
        if len(batches) > 1:
            if is_text_mode:
                ...
                batch_header = f"**批次 {i}/{len(batches)}**\n\n"
...
class NewsAnalyzer:
    ...
    def _has_notification_configured(self) -> bool:
        """检查是否配置了企业微信通知渠道"""
        return bool(CONFIG["WEWORK_WEBHOOK_URL"])
